export function buildSearchQuery(options, vectorEnabled) {
    const values = [];
    const where = [];
    ...
    if (queryText) {
        ...
        where.push(`to_tsvector('english', coalesce(e.title, '') || ' ' || e.content) @@ websearch_to_tsquery('english', $${queryParamIndex})`);
    }
    const mode = options.searchMode ?? "fts";
    const useVector = vectorEnabled &&
